...
class TeamStats:
    ...
    def form_last_5(self):
        # Maksymalnie 15 punktów w 5 meczach
...
def update_elo(home_stats, away_stats, result):
    # Aktualizacja ratingu Elo po meczu
    k = 24
...
class CustomDecisionTree:
    ...
    def find_thresholds(self, values):
        # Zamiast sprawdzać wszystkie wartości, bierzemy kilka progów z percentyli
...
class CustomRandomForest:
    ...
    def bootstrap_sample(self, X, y):
        # Losowanie próbek z powtórzeniami
        n_samples = len(X)
...
class RandomForestMatchModel:
    ...
    def prepare_training_data(self):
        print("1. Wczytywanie danych do Random Forest...")
    ...
    def fit(self):
        ...
        print("3. Trenowanie Random Forest...")
        self.model.fit(X_train, y_train)

        print("4. Testowanie Random Forest...")
        predictions = self.model.predict(X_test)

        correct = np.sum(predictions == y_test)
        self.accuracy = correct / len(y_test)

        print(f"Random Forest accuracy: {self.accuracy * 100:.2f}%")
